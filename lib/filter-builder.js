...
function getFilterConditions(keyword, vote_ids, search_locations, us_states, countries, years, start_date, end_date) {
  ...
  // Build geographic location conditions.
  // If states are selected they override the US country entry — combine as OR:
  //   (country = CN) OR (region = New York OR region = California)
  // If no states, filter by countries as-is.
  const hasStates = us_states && us_states.length > 0;
  const effectiveCountries = hasStates
    ? (countries || []).filter(c => c !== 'US')
    : (countries || []);

  const locationParts = [];

  if (effectiveCountries.length > 0) {
    const condition = effectiveCountries
      .map(c => `s.search_country_code = '${c}'`)
      .join(' OR ');
    locationParts.push(effectiveCountries.length > 1 ? `(${condition})` : condition);
  }
  ...
  if (hasStates) {
    const condition = us_states
      ...
      .join(' OR ');
    locationParts.push(us_states.length > 1 ? `(${condition})` : condition);
  }

  if (locationParts.length > 1) {
    conditions.push(`(${locationParts.join(' OR ')})`);
  } else if (locationParts.length === 1) {
    conditions.push(locationParts[0]);
  }
